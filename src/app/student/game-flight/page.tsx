...
export default function StudentGamePage() {
  ...
  const renderGame = useCallback((state: GameState) => {
    ...
    ctx.font = "16px Arial";
    ctx.textAlign = "left";
    ctx.fillText(`完成單字: ${state.wordsCompleted}${state.wordsCompleted >= WORDS_FOR_DOUBLE_SCORE ? " ✓雙倍!" : ` / ${WORDS_FOR_DOUBLE_SCORE}`}`, 10, CANVAS_HEIGHT - 55);
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      if (state.bossAttackCooldown <= 0) {
        ...
        if (attackType < 0.4) {
          state.bullets.push({
            ...
            damage: 15, isPlayer: false, width: 30, height: 30, speed: BOSS_BULLET_SPEED, color: "#ff4444", type: "large"
          });
          ...
          for (let i = -2; i <= 2; i++) {
            state.bullets.push({
              ...
              damage: 8, isPlayer: false, width: 12, height: 12, speed: BOSS_BULLET_SPEED * 0.8, color: "#ff8800", type: "spread", targetX: i * 1.5
            });
          }
          ...
          state.bullets.push({
            ...
            damage: 12, isPlayer: false, width: 16, height: 16, speed: BOSS_BULLET_SPEED * 0.4, color: "#ff00ff", type: "tracking",
            createdAt: now
          });
      ...
      state.fallingLetters = state.fallingLetters.filter((letter) => {
        if (
          letter.x < state.playerX + PLAYER_WIDTH && letter.x + 28 > state.playerX &&
          letter.y < state.playerY + PLAYER_HEIGHT && letter.y + 28 > state.playerY
        ) {
          const nextIndex = state.collectedLetters.length;
      ...
      {!gameStarted ? (
        <Paper sx={{ p: 3, maxWidth: 600, mx: "auto" }}>
          ...
          <Box sx={{ mb: 2, p: 2, bgcolor: "grey.100", borderRadius: 1 }}>
            ...
            <Typography variant="body2" color="text.secondary">
              攻擊速度: {selectedDifficulty === "easy" ? "慢" : selectedDifficulty === "normal" ? "普通" : selectedDifficulty === "hard" ? "快" : "極快"}
            </Typography>
          </Box>
          ...
          <Button
            ...
            sx={{ 
              py: 2,
              backgroundColor: DIFFICULTY_SETTINGS[selectedDifficulty].color,
              "&:hover": {
                backgroundColor: DIFFICULTY_SETTINGS[selectedDifficulty].color,
      ...
      <Dialog open={showResult} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ textAlign: "center", py: 2 }}>
            ...
            <Chip 
              label={DIFFICULTY_SETTINGS[displayState.difficulty].name}
              sx={{ 
                backgroundColor: DIFFICULTY_SETTINGS[displayState.difficulty].color,
                color: "#fff",
                mb: 2
              }}
            />
